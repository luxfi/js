...
export class UTXOSet extends StandardUTXOSet<UTXO> {
   ...
   * @param luxAssetID {@link https://github.com/feross/buffer|Buffer} of the asset ID for LUX
   ...
   * @param changeAddresses An array of addresses as {@link https://github.com/feross/buffer|Buffer} who gets the change leftover of the LUX
  ...
  buildExportTx = (
    ...
    blockchainID: Buffer,
    amount: BN,
    luxAssetID: Buffer, // TODO: rename this to amountAssetID
    toAddresses: Buffer[],
    fromAddresses: Buffer[],
    changeAddresses: Buffer[] = undefined,
   ...
   * @param startTime The Unix time when the validator starts validating the Primary Network.
   * @param endTime The Unix time when the validator stops validating the Primary Network (and staked LUX is returned).
   * @param stakeAmount A {@link https://github.com/indutny/bn.js/|BN} for the amount of stake to be delegated in nLUX.
  ...
  buildAddDelegatorTx = (
    ...
    if (luxAssetID.toString("hex") === feeAssetID.toString("hex")) {
      aad.addAssetAmount(luxAssetID, stakeAmount, fee)
    } else {
      aad.addAssetAmount(luxAssetID, stakeAmount, zero)
      if (this._feeCheck(fee, feeAssetID)) {
        aad.addAssetAmount(feeAssetID, zero, fee)
      }
    }
