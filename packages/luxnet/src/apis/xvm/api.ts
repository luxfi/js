...
/**
 * Class for interacting with a node endpoint that is using the XVM.
 *
 * @category RPCAPIs
 *
 ...
 */
export class XVMAPI extends JRPCAPI {
  /**
   * @ignore
   */
  ...
  getBalance = async (
    ...
    if (typeof this.parseAddress(address) === "undefined") {
      ...
      throw new AddressError(
        "Error - XVMAPI.getBalance: Invalid address format"
      )
    }
  ...
  createNFTAsset = async (
    ...
    if (typeof changeAddr !== "undefined") {
      if (typeof this.parseAddress(changeAddr) === "undefined") {
        /* istanbul ignore next */
        throw new AddressError(
          "Error - XVMAPI.createNFTAsset: Invalid address format"
        )
      }
  ...
  /**
   * Mint non-fungible tokens which were created with XVMAPI.createNFTAsset
   *
   * @param username The user paying the transaction fee (in $LUX) for asset creation
   * @param password The password for the user paying the transaction fee (in $LUX) for asset creation
   * @param from Optional. An array of addresses managed by the node's keystore for this blockchain which will fund this transaction
  ...
  mintNFT = async (
    ...
    if (typeof this.parseAddress(to) === "undefined") {
      /* istanbul ignore next */
      throw new AddressError("Error - XVMAPI.mintNFT: Invalid address format")
    }
  ...
  sendNFT = async (
    ...
    if (typeof this.parseAddress(to) === "undefined") {
      /* istanbul ignore next */
      throw new AddressError("Error - XVMAPI.sendNFT: Invalid address format")
    }
  ...
  exportKey = async (
    ...
    if (typeof this.parseAddress(address) === "undefined") {
      /* istanbul ignore next */
      throw new AddressError("Error - XVMAPI.exportKey: Invalid address format")
    }
  ...
  getAllBalances = async (address: string): Promise<object[]> => {
    if (typeof this.parseAddress(address) === "undefined") {
      /* istanbul ignore next */
      throw new AddressError(
        "Error - XVMAPI.getAllBalances: Invalid address format"
      )
    }
  ...
  buildBaseTx = async (
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      ...
      throw new GooseEggCheckError(
        "Error - XVMAPI.buildBaseTx:Failed Goose Egg Check"
      )
    }
  ...
  buildNFTTransferTx = async (
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - XVMAPI.buildNFTTransferTx:Failed Goose Egg Check"
      )
    }
  ...
  buildImportTx = async (
    ...
    if (typeof sourceChain === "undefined") {
      throw new ChainIdError(
        "Error - XVMAPI.buildImportTx: Source ChainID is undefined."
      )
      ...
      throw new ChainIdError(
        "Error - XVMAPI.buildImportTx: Invalid destinationChain type: " +
          typeof sourceChain
      )
    }
    ...
    if (atomics.length === 0) {
      throw new NoAtomicUTXOsError(
        "Error - XVMAPI.buildImportTx: No atomic UTXOs to import from " +
          srcChain +
          " using addresses: " +
          ownerAddresses.join(", ")
      )
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - XVMAPI.buildImportTx:Failed Goose Egg Check"
      )
    }
  ...
  buildExportTx = async (
    ...
    if (Object.keys(prefixes).length !== 1) {
      throw new AddressError(
        "Error - XVMAPI.buildExportTx: To addresses must have the same chainID prefix."
      )
    }

    if (typeof destinationChain === "undefined") {
      throw new ChainIdError(
        "Error - XVMAPI.buildExportTx: Destination ChainID is undefined."
      )
    } else if (typeof destinationChain === "string") {
      destinationChain = bintools.cb58Decode(destinationChain) //
    } else if (!(destinationChain instanceof Buffer)) {
      throw new ChainIdError(
        "Error - XVMAPI.buildExportTx: Invalid destinationChain type: " +
          typeof destinationChain
      )
    }
    if (destinationChain.length !== 32) {
      throw new ChainIdError(
        "Error - XVMAPI.buildExportTx: Destination ChainID must be 32 bytes in length."
      )
    }
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - XVMAPI.buildExportTx:Failed Goose Egg Check"
      )
    }
  ...
  buildCreateAssetTx = async (
    ...
    if (symbol.length > XVMConstants.SYMBOLMAXLEN) {
      throw new SymbolError(
        "Error - XVMAPI.buildCreateAssetTx: Symbols may not exceed length of " +
          XVMConstants.SYMBOLMAXLEN
      )
    }
    if (name.length > XVMConstants.ASSETNAMELEN) {
      throw new NameError(
        "Error - XVMAPI.buildCreateAssetTx: Names may not exceed length of " +
          XVMConstants.ASSETNAMELEN
      )
    }
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx, fee))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - XVMAPI.buildCreateAssetTx:Failed Goose Egg Check"
      )
    }
  ...
  buildSECPMintTx = async (
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - XVMAPI.buildSECPMintTx:Failed Goose Egg Check"
      )
    }
    return builtUnsignedTx
  }
  ...
  buildCreateNFTAssetTx = async (
    ...
    if (name.length > XVMConstants.ASSETNAMELEN) {
      /* istanbul ignore next */
      throw new NameError(
        "Error - XVMAPI.buildCreateNFTAssetTx: Names may not exceed length of " +
          XVMConstants.ASSETNAMELEN
      )
    }
    if (symbol.length > XVMConstants.SYMBOLMAXLEN) {
      /* istanbul ignore next */
      throw new SymbolError(
        "Error - XVMAPI.buildCreateNFTAssetTx: Symbols may not exceed length of " +
          XVMConstants.SYMBOLMAXLEN
      )
    }
    const networkID: number = this.core.getNetworkID()
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx, creationTxFee))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - XVMAPI.buildCreateNFTAssetTx:Failed Goose Egg Check"
      )
    }
    return builtUnsignedTx
  }
  ...
  buildCreateNFTMintTx = async (
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - XVMAPI.buildCreateNFTMintTx:Failed Goose Egg Check"
      )
    }
    return builtUnsignedTx
  }
  ...
  issueTx = async (tx: string | Buffer | Tx): Promise<string> => {
    ...
    if (typeof tx === "string") {
      ...
      throw new TransactionError(
        "Error - XVMAPI.issueTx: provided tx is not expected type of string, Buffer, or Tx"
      )
    }
  ...
  send = async (
    ...
    if (typeof this.parseAddress(to) === "undefined") {
      /* istanbul ignore next */
      throw new AddressError("Error - XVMAPI.send: Invalid address format")
    }
    ...
    sendOutputs.forEach(
      (output: {
        ...
        if (typeof this.parseAddress(output.to) === "undefined") {
          /* istanbul ignore next */
          throw new AddressError(
            "Error - XVMAPI.sendMultiple: Invalid address format"
          )
        }
      ...
      for (let i: number = 0; i < addresses.length; i++) {
        if (typeof addresses[`${i}`] === "string") {
          if (
            ...
            /* istanbul ignore next */
            throw new AddressError(
              "Error - XVMAPI.${caller}: Invalid address format"
            )
          }
