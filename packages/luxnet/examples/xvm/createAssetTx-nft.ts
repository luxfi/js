...
const memo: Buffer = Buffer.from("XVM manual CreateAssetTx to create an NFT")
...
const main = async (): Promise<any> => {
  ...
  utxos.forEach((utxo: UTXO): void => {
    ...
    if (
      outputID === 7 &&
      assetID.toString("hex") === luxAssetIDBuf.toString("hex")
    ) {
  ...
  for (let i: number = 0; i <= 5; i++) {
    ...
    initialStates.addOutput(nftMintOutput, XVMConstants.NFTFXID)
  }
