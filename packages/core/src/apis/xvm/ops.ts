...
export const SelectOperationClass = (
  ...
  if (
    opid === XVMConstants.SECPMINTOPID ||
    opid === XVMConstants.SECPMINTOPID_CODECONE
  ) {
    return new SECPMintOperation(...args)
  } else if (
    opid === XVMConstants.NFTMINTOPID ||
    opid === XVMConstants.NFTMINTOPID_CODECONE
  ) {
    return new NFTMintOperation(...args)
  } else if (
    opid === XVMConstants.NFTXFEROPID ||
    opid === XVMConstants.NFTXFEROPID_CODECONE
  ) {
...
export class TransferableOperation extends Serializable {
  ...
  constructor(
    ...
    if (
      ...
      assetID.length === XVMConstants.ASSETIDLEN &&
...
export class SECPMintOperation extends Operation {
  ...
  protected _typeID =
    this._codecID === 0
      ? XVMConstants.SECPMINTOPID
      : XVMConstants.SECPMINTOPID_CODECONE
  ...
  getCredentialID(): number {
    if (this._codecID === 0) {
      return XVMConstants.SECPCREDENTIAL
    } else if (this._codecID === 1) {
      return XVMConstants.SECPCREDENTIAL_CODECONE
    }
  }
...
export class NFTMintOperation extends Operation {
  ...
  getCredentialID = (): number => {
    if (this._codecID === 0) {
      return XVMConstants.NFTCREDENTIAL
    } else if (this._codecID === 1) {
      return XVMConstants.NFTCREDENTIAL_CODECONE
    }
  }
