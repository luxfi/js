...
const memo: Buffer = Buffer.from(
  "Manually Import LUX and ANT to the X-Chain from the C-Chain"
)
// Uncomment for codecID 00 01
// const codecID: number = 1

const main = async (): Promise<any> => {
  const xvmUTXOResponse: any = await xchain.getUTXOs(
    xAddressStrings,
    cChainBlockchainID
  )
  ...
  utxos.forEach((utxo: UTXO) => {
    ...
    if (luxAssetIDBuf.toString("hex") === assetID.toString("hex")) {
