...
export const SelectOutputClass = (outputid: number, ...args: any[]): Output => {
  if (
    outputid === XVMConstants.SECPXFEROUTPUTID ||
    outputid === XVMConstants.SECPXFEROUTPUTID_CODECONE
  ) {
    return new SECPTransferOutput(...args)
  } else if (
    outputid === XVMConstants.SECPMINTOUTPUTID ||
    outputid === XVMConstants.SECPMINTOUTPUTID_CODECONE
  ) {
    return new SECPMintOutput(...args)
  } else if (
    outputid === XVMConstants.NFTMINTOUTPUTID ||
    outputid === XVMConstants.NFTMINTOUTPUTID_CODECONE
  ) {
    return new NFTMintOutput(...args)
  } else if (
    outputid === XVMConstants.NFTXFEROUTPUTID ||
    outputid === XVMConstants.NFTXFEROUTPUTID_CODECONE
  ) {
...
export class TransferableOutput extends StandardTransferableOutput {
  ...
  fromBuffer(bytes: Buffer, offset: number = 0): number {
    this.assetID = bintools.copyFrom(
      bytes,
      offset,
      offset + XVMConstants.ASSETIDLEN
    )
    offset += XVMConstants.ASSETIDLEN
...
export class SECPTransferOutput extends AmountOutput {
  ...
  protected _typeID =
    this._codecID === 0
      ? XVMConstants.SECPXFEROUTPUTID
      : XVMConstants.SECPXFEROUTPUTID_CODECONE
...
export class SECPMintOutput extends Output {
  ...
  protected _typeID =
    this._codecID === 0
      ? XVMConstants.SECPMINTOUTPUTID
      : XVMConstants.SECPMINTOUTPUTID_CODECONE
...
export class NFTMintOutput extends NFTOutput {
  ...
  protected _typeID =
    this._codecID === 0
      ? XVMConstants.NFTMINTOUTPUTID
      : XVMConstants.NFTMINTOUTPUTID_CODECONE
...
export class NFTTransferOutput extends NFTOutput {
  ...
  protected _typeID =
    this._codecID === 0
      ? XVMConstants.NFTXFEROUTPUTID
      : XVMConstants.NFTXFEROUTPUTID_CODECONE
